...
PrismLateral::PrismLateral(int* tmpNodeIds, vector<Node*>& Nodes, int CurrId){
	nNodes = 6;
	nDim = 3;
	Id = CurrId;
	NodeIds = new int[6];
...
void PrismLateral::setRefShapePosBuffers(){
	const int n = nNodes;
	const int dim = nDim;
...
void PrismLateral::setStiffnessMatrixBuffers(){
...
void PrismLateral::setTissueCoordsRotationsBuffers(){
...
void PrismLateral::getCurrRelaxedShape(boost::numeric::ublas::matrix<double> & CurrRelaxedShape){
	using namespace boost::numeric::ublas;
	for (int i =0; i<nNodes; ++i){
...
void PrismLateral::setShapeFunctionDerivatives(boost::numeric::ublas::matrix<double> &ShapeFuncDer, double eta, double zeta, double nu){
...
void PrismLateral::setShapeFunctionDerivativeStack(boost::numeric::ublas::matrix<double> &ShapeFuncDer,boost::numeric::ublas::matrix<double> &ShapeFuncDerStack){
	int n = nNodes;
	int dim = nDim;
	for (int i=0; i<n;++i){
...
void PrismLateral::calculateReferenceStiffnessMatrix(){
	const int n = nNodes;
	const int dim = nDim;
...
void PrismLateral::calculateCurrk(boost::numeric::ublas::matrix<double>& currk, boost::numeric::ublas::matrix<double>& currB, boost::numeric::ublas::matrix<double>& currBE, double eta, double zeta, double nu){
	const int n = nNodes;
	const int dim = nDim;
...
void PrismLateral::updateAlignmentTurn(){
	if (alingmentTurn == 0){
		alingmentTurn = 1;
	}
	else{
...
void PrismLateral::updateReferenceShapeBaseFromBuffer(){
...
void PrismLateral::resetBuffersAfterGrowth(){
	for (int i = 0 ; i < nNodes; ++i ){
		for (int j=0; j < nDim; ++j){
			RefShapePosBottomAlignedBuffer[i][j] = ReferenceShape->Positions[i][j];
			RefShapePosTopAlignedBuffer[i][j] =ReferenceShape->Positions[i][j];
