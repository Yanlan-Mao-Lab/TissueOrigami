...
using namespace std;
 ...
 float** GLWidget::getElementColourList(int i){
	...
	if(DisplayStrains){
		...
		float TmpStrainMag =0.0, GrowthStrainMag=0;
		Sim01->Elements[i]->getStrain(StrainToDisplay, TmpStrainMag);
		Sim01->Elements[i]->getGrowthStrain(StrainToDisplay, GrowthStrainMag);
		float StrainMag = TmpStrainMag - GrowthStrainMag;
 ...
 void GLWidget::drawPrism(int i){
	...
	bool drawGrowthCoordinatesystem = true;
	if (drawGrowthCoordinatesystem){
		drawGrowthCoordSystemPrism(i);
	}
	if (drawNormals){
 ...
 void GLWidget::drawGrowthCoordSystemPrism(int i){
	glLineWidth(ReferenceLineThickness);
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	...
	glBegin(GL_LINES);
		glColor3f(1,0,0);
		tip[0] = centre[0] + Sim01->Elements[i]->GrowthCoordinateSystem[0];
		tip[1] = centre[1] + Sim01->Elements[i]->GrowthCoordinateSystem[1];
		tip[2] = centre[2] + Sim01->Elements[i]->GrowthCoordinateSystem[2];

		glVertex3f( centre[0], centre[1], centre[2]);
		glVertex3f( tip[0] , tip[1], tip[2]);
		glColor3f(0,1,0);
		tip[0] = centre[0] + Sim01->Elements[i]->GrowthCoordinateSystem[3];
		tip[1] = centre[1] + Sim01->Elements[i]->GrowthCoordinateSystem[4];
		tip[2] = centre[2] + Sim01->Elements[i]->GrowthCoordinateSystem[5];

		glVertex3f( centre[0], centre[1], centre[2]);
		glVertex3f( tip[0] , tip[1], tip[2]);
		glColor3f(0,0,1);
		tip[0] = centre[0] + Sim01->Elements[i]->GrowthCoordinateSystem[6];
		tip[1] = centre[1] + Sim01->Elements[i]->GrowthCoordinateSystem[7];
		tip[2] = centre[2] + Sim01->Elements[i]->GrowthCoordinateSystem[8];
		glVertex3f( centre[0], centre[1], centre[2]);
		glVertex3f( tip[0] , tip[1], tip[2]);
	glEnd();
 }
