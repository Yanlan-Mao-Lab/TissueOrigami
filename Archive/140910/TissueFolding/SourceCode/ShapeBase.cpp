...
void 	ShapeBase::getStrainColour(int type, float* StrainColour){
	...
	//cout<<"Strain To Display: "<<Strain(0)<<" "<<Strain(1)<<" "<<Strain(2)<<" colour: "<< StrainColour[0]<<" "<<StrainColour[1]<<" "<<StrainColour[2]<<endl;
}
...
/*void 	ShapeBase::growShape(double* growthscale){
	for (int i=0;i<3;++i){
		CurrPlasticStrainAddition[i]  = CurrPlasticStrainAddition[i] + growthscale[i];
	}
}

void 	ShapeBase::clearCurrentPlasticStrainAdd(){
	for (int i=0;i<3;++i){
		CurrPlasticStrainAddition[i]  = 0.0;
	}
}
...
void 	ShapeBase::growShape(double* growthscale){
	...
	//Now I have the growth coordinate system, I need to calculate the rotation matrix to rotate
	//current growth strains to the coordinate system of the reference prism
	calculateGrowthRotationMatrixToReference();
	//Now I have the rotation matrix and its transpose, I will rotate the strains:
	updateLocalGrowthStrain(CurrPlasticStrainAddition);
	//Now I have the new local growth strain ready, i need to transform it to world axes:
	//I already have the rotation matrices that I need:
	boost::numeric::ublas::matrix<double> LocalPlasticStrainsMat(3,3);
	LocalPlasticStrainsMat(0,0)= LocalPlasticStrains[0];
	LocalPlasticStrainsMat(1,1)= LocalPlasticStrains[1];
	LocalPlasticStrainsMat(2,2)= LocalPlasticStrains[2];
	LocalPlasticStrainsMat(0,1)= LocalPlasticStrains[3];
	LocalPlasticStrainsMat(1,0)= LocalPlasticStrains[3];
	LocalPlasticStrainsMat(0,2)= LocalPlasticStrains[5];
	LocalPlasticStrainsMat(2,0)= LocalPlasticStrains[5];
	LocalPlasticStrainsMat(1,2)= LocalPlasticStrains[4];
	LocalPlasticStrainsMat(2,1)= LocalPlasticStrains[4];

	boost::numeric::ublas::matrix<double> tmpMat1(3,3);
	boost::numeric::ublas::matrix<double> tmpMat2(3,3);
	boost::numeric::ublas::matrix<double> TotalRotation(3,3);
	axpy_prod(GrowthToWorldRotMat,GrowthToRefRotMatT,TotalRotation);
	boost::numeric::ublas::matrix<double> TotalRotationT(3,3);
	TotalRotationT = trans(TotalRotation);
	axpy_prod(TotalRotation,LocalPlasticStrainsMat,tmpMat1);
	axpy_prod(tmpMat1,TotalRotationT,tmpMat2);
	GrowthStrain(0)= tmpMat2(0,0);
	GrowthStrain(1)= tmpMat2(1,1);
	GrowthStrain(2)= tmpMat2(2,2);
	GrowthStrain(3)= tmpMat2(1,0);
	GrowthStrain(4)= tmpMat2(1,2);
	GrowthStrain(5)= tmpMat2(2,0);
	//displayMatrix(LocalPlasticStrainsMat,"LocalPlasticStrainsMat");
	//cout<<"growthscale: "<<endl<<growthscale[0]<<"	"<<growthscale[1]<<"	"<<growthscale[2]<<endl;
...
void 	ShapeBase::updateLocalGrowthStrain(double* strainsToAdd){
	using namespace boost::numeric::ublas;
	boost::numeric::ublas::matrix<double> currStrains;
	currStrains = boost::numeric::ublas::zero_matrix<double>(3,3);
	currStrains(0,0)= strainsToAdd[0];
	currStrains(1,1)= strainsToAdd[1];
	currStrains(2,2)= strainsToAdd[2];
	//currStrains(0,1)= strainsToAdd[3];
	//currStrains(1,0)= strainsToAdd[3];
	//currStrains(0,2)= strainsToAdd[5];
	//currStrains(2,0)= strainsToAdd[5];
	//currStrains(1,2)= strainsToAdd[4];
	//currStrains(2,1)= strainsToAdd[4];
	matrix<double> tmpMat1(3,3);
	matrix<double> LocalStrainsToAddMat(3,3);
	axpy_prod(GrowthToRefRotMat,currStrains,tmpMat1);
	axpy_prod(tmpMat1,GrowthToRefRotMatT,LocalStrainsToAddMat);
	boost::numeric::ublas::vector<double> LocalStrainsToAdd(6);
	LocalStrainsToAdd(0)= LocalStrainsToAddMat(0,0);
	LocalStrainsToAdd(1)= LocalStrainsToAddMat(1,1);
	LocalStrainsToAdd(2)= LocalStrainsToAddMat(2,2);
	LocalStrainsToAdd(3)= LocalStrainsToAddMat(1,0);
	LocalStrainsToAdd(4)= LocalStrainsToAddMat(1,2);
	LocalStrainsToAdd(5)= LocalStrainsToAddMat(2,0);
	displayMatrix(GrowthToRefRotMat,"GrowthToRefRotMat");
	displayMatrix(currStrains,"currStrains");
	displayMatrix(LocalPlasticStrains,"LocalPlasticStrains");
	displayMatrix(LocalStrainsToAddMat,"LocalStrainsToAddMat");
	displayMatrix(LocalStrainsToAdd,"LocalStrainsToAdd");
	LocalPlasticStrains = LocalPlasticStrains + LocalStrainsToAdd;
	displayMatrix(LocalPlasticStrains,"LocalPlasticStrains");
}
...
void ShapeBase::calculateGrowthCoordinateSystem(){
	...
	updateGrowthToWorldRotMat(rotMat);
	rotateWorldsCoordinatesByRotationMatrix(GrowthCoordinateSystem,rotMat);
	cout<<"GrowthCoordinateSystem:"<<endl;
	cout<<GrowthCoordinateSystem[0]<<" 	"<<GrowthCoordinateSystem[1]<<" 	"<<GrowthCoordinateSystem[2]<<endl;
	cout<<GrowthCoordinateSystem[3]<<" 	"<<GrowthCoordinateSystem[4]<<" 	"<<GrowthCoordinateSystem[5]<<endl;
	cout<<GrowthCoordinateSystem[6]<<" 	"<<GrowthCoordinateSystem[7]<<" 	"<<GrowthCoordinateSystem[8]<<endl;
	delete[] u;
	delete[] v;
	delete[] rotAx;
	delete[] rotMat;
}
void ShapeBase::updateGrowthToWorldRotMat(double* rotMat){
...
void ShapeBase::calculateGrowthRotationMatrixToReference(){
	...
	double* v = new double[3];
	calculateXVecForGrowhtAlignment(v);
	double* u = new double[3];
	u[0]=GrowthCoordinateSystem[0];
	u[1]=GrowthCoordinateSystem[1];
	u[2]=GrowthCoordinateSystem[2];
	double c, s;
	calculateRotationAngleSinCos(u,v,c,s);
	...
	rotMat = new double[9]; //matrix is written in one row
	calculateRotationAxis(u,v,rotAx);
	cout<<"u: "<<u[0]<<" "<<u[1]<<" "<<u[2]<<" v: "<<v[0]<<" "<<v[1]<<" "<<v[2]<<"RotAx: "<<rotAx[0]<<" "<<rotAx[1]<<" "<<rotAx[2]<<endl;
	constructRotationMatrix(c,s,rotAx,rotMat);
	GrowthToRefRotMat(0,0)=rotMat[0];
	GrowthToRefRotMat(0,1)=rotMat[1];
	GrowthToRefRotMat(0,2)=rotMat[2];
	GrowthToRefRotMat(1,0)=rotMat[3];
	GrowthToRefRotMat(1,1)=rotMat[4];
	GrowthToRefRotMat(1,2)=rotMat[5];
	GrowthToRefRotMat(2,0)=rotMat[6];
	GrowthToRefRotMat(2,1)=rotMat[7];
	GrowthToRefRotMat(2,2)=rotMat[8];
	GrowthToRefRotMatT = trans(GrowthToRefRotMat);
	delete[] u;
	delete[] v;
	delete[] rotAx;
	delete[] rotMat;
