...
bool Simulation::initiateSystem(){
	...
	ModInp->Sim=this;
	ModInp->parameterFileName =  "/home/melda/Desktop/modelinput";
	ModInp->meshFileName = "NA";
...
void Simulation::assignPhysicalParameters(){
	double E = 10.0, v = 0.3;
	double ApicalVisc = 10.0, BasalVisc=100.0;
	for (int i=0; i<Elements.size();++i){
		float vnoise1 = rand() % 50;
		vnoise1 = 1 + vnoise1/100;
		float vnoise2 = rand() % 50;
		vnoise2 = 1 + vnoise2/100;
		Elements[i]->setElasticProperties(E*vnoise1,v);
		Elements[i]->setViscosity(ApicalVisc*vnoise2, BasalVisc*vnoise2, Nodes);
	}
}
...
void Simulation::RunOneStep(){
	...
	if (time == 0.0){
		//moving first 3 nodes down in z
		for (int i = 0; i<3; ++i){
				Nodes[i]->Position[2] -=2;
		}
		//Rotate the whole system around x axis, and I do not want to align the references on display for now:
		//rotation of 90 degrees around x-axis:
		//[1 0 0;0 0 -1; 0,1,0]
		//
		double RM[3][3];
		//rotating 90 degrees in x
		RM[0][0]=1;RM[0][1]=0;RM[0][2]=0;
		RM[1][0]=0;RM[1][1]=0;RM[1][2]=1;
		RM[2][0]=0;RM[2][1]=-1;RM[2][2]=0;
		int nNodes = Nodes.size();
		for (int i = 0; i<nNodes; ++i){
			double x = RM[0][0]*Nodes[i]->Position[0]+RM[0][1]*Nodes[i]->Position[1]+RM[0][2]*Nodes[i]->Position[2];
			double y = RM[1][0]*Nodes[i]->Position[0]+RM[1][1]*Nodes[i]->Position[1]+RM[1][2]*Nodes[i]->Position[2];
			double z = RM[2][0]*Nodes[i]->Position[0]+RM[2][1]*Nodes[i]->Position[1]+RM[2][2]*Nodes[i]->Position[2];
			Nodes[i]->Position[0]=x; Nodes[i]->Position[1]=y; Nodes[i]->Position[2]=z;
		}
		//rotating 90 degrees in y:
		RM[0][0]=0;RM[0][1]=0;RM[0][2]=1;
		RM[1][0]=0;RM[1][1]=1;RM[1][2]=0;
		RM[2][0]=-1;RM[2][1]=0;RM[2][2]=0;
	...
	//	growSystem(2);
	//}
	//size doubles every half hour, then rate in one time step: 2.0 / (10min) * dt,
	//then the scale becomes : 2/(10min) * dt + 1.0;
...
void Simulation::growSystem(float scale){
	...
	double rangex = 1;
	double rangey = 5;
	double range2x = rangex*rangex;
	double range2y = rangey*rangey;
	...
	for ( int i = 0; i < n; ++i ){
		...
		//ellipse - like surface:
		/*double dmag2x = d[0]*d[0];
		double dmag2y = d[1]*d[1];
		double S = 1.0;
		bool growth = false;
		if (dmag2x < range2x && dmag2y < range2y){
			double dmag = pow(dmag2x+dmag2y,0.5);
			S += (scale-1.0) * (rangex*rangey-dmag)/(rangex*rangey);
			growth = true;
		}
		if (growth){
			Elements[i]->growShape(S);
