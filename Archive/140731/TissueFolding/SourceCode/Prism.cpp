...
Prism::Prism(int* tmpNodeIds, vector<Node*>& Nodes, int CurrId){
	...
	setCoeffMat();
	CurrentNormal = new double[3];
	CurrentNormal[0]=0.0;CurrentNormal[1]=0.0;CurrentNormal[2]=0.0;
	...
	for (int etaiter = 0; etaiter<4; ++etaiter){
		...
		for(int nuiter = 0; nuiter<4; ++nuiter){
			...
			for (int zetaiter = 0; zetaiter<4; ++zetaiter){
				...
				calculateCurrk(currk, currB,eta,zeta,nu);
				kSumZeta = kSumZeta + GaussCoeff[zetaiter]   * currk;
				BSumZeta = BSumZeta + GaussCoeff[zetaiter]/2 * currB;
			}
...
void Prism::calculateCurrk(boost::numeric::ublas::matrix<double>& currk, boost::numeric::ublas::matrix<double>& currB, double eta, double zeta, double nu){
	const int n = nNodes;
	const int dim = nDim;
	...
	//Generating currk:
	matrix<double> tmpMat2(dim*n,6);
	matrix<double> currBT = trans(currB);

	boost::numeric::ublas::axpy_prod(currBT,detJ*D,tmpMat2);
	boost::numeric::ublas::axpy_prod(tmpMat2,currB,currk);
...
void Prism::calculateNormal(){
	bool alingmentToBottom = true;
	if (alingmentToBottom){
...
void Prism::calculateNormalToBottom(){
	...
	double* vec2;
	vec2 = new double[3];
	vec2[0] = Positions[2][0] - Positions[1][0];
	vec2[1] = Positions[2][1] - Positions[1][1];
	vec2[2] = Positions[2][2] - Positions[1][2];

	//normal vector to the triangle
	CurrentNormal = crossProduct3D(vec1,vec2);
