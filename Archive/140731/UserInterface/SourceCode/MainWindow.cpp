...
void MainWindow::setStrainDisplayMenu(QGridLayout *SelectionDisplayGrid){
	...
	StrainComboBox->addItem("Strain Summation");
	StrainComboBox->addItem("Strain in x");
	StrainComboBox->addItem("Strain in y");
	StrainComboBox->addItem("Strain in z");
...
void MainWindow::timerSimulationStep(){
	...
	//while (Sim01->time<0.01){
		//cout<<"Display Strain Ranges: "<<MainGLWidget->DisplayStrainRange[0]<<" "<<MainGLWidget->DisplayStrainRange[1]<<endl;
		cout<<"running step"<<endl;
		Sim01->RunOneStep();
		//cout<<"finalised running step"<<endl;
		MainGLWidget->update();
		//MainGLWidget->takeScreenshot();
	//}
}
