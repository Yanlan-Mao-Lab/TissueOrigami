...
using namespace std;
 ...
 GLWidget::GLWidget(QWidget *parent) : QGLWidget(parent)
 {
     ...
     DisplayPysPropRange[3][0] = -0.5; DisplayPysPropRange[3][1] = 1.0;
     ...
     DisplayPysPropBounds[3][0] = -1.0; DisplayPysPropBounds[3][1] = 0.0;
     DisplayPysPropBounds[3][2] = 0.0; DisplayPysPropBounds[3][3] = 2.0;
 ...
 float** GLWidget::getElementColourList(int i){
	...
	if(DisplayStrains){
		...
		float StrainMag =0.0;
		Sim01->Elements[i]->getStrain(StrainToDisplay, StrainMag);
 ...
 void GLWidget::drawNormalToPrism(int i){
 	 float centre[3];
 	 centre[0] = Sim01->Elements[i]->Positions[0][0] + Sim01->Elements[i]->Positions[1][0] +Sim01->Elements[i]->Positions[2][0];
 	 centre[1] = Sim01->Elements[i]->Positions[0][1] + Sim01->Elements[i]->Positions[1][1] +Sim01->Elements[i]->Positions[2][1];
 	 centre[2] = Sim01->Elements[i]->Positions[0][2] + Sim01->Elements[i]->Positions[1][2] +Sim01->Elements[i]->Positions[2][2];
 	 centre[0] /=3.0;
 	 centre[1] /=3.0;
 	 centre[2] /=3.0;
 	 float normalTip[3];
 	 normalTip[0] = centre[0] + Sim01->Elements[i]->CurrentNormal[0];
 	 normalTip[1] = centre[1] + Sim01->Elements[i]->CurrentNormal[1];
 	 normalTip[2] = centre[2] + Sim01->Elements[i]->CurrentNormal[2];

 	 //cout<<"drawing normal to prism: "<<i<<" - centre: "<<centre[0]<<" "<<centre[1]<<" "<<centre[2]<<" tip: "<<normalTip[0]<<" "<<normalTip[1]<<" "<<normalTip[2]<<endl;
 ...
 void GLWidget::mouseMoveEvent(QMouseEvent *event)
 {
	 if (MouseButton==0){
		 ...
		 cout<<"initial - last: "<<initialPos[0]<<"	"<<initialPos[1]<<"	"<<initialPos[2]<<"	"<<finalPos[0]<<"	"<<finalPos[1]<<"	"<<finalPos[2]<<endl;
		 float r = 20;
		 float r2 = r*r;
...
void GLWidget::takeScreenshot(){
	...
	//QPixmap::grabWindow(this->winId() ).save( "/home/Melda/Desktop/test.jpg" );
	//QPixmap  p = QPixmap::grabWindow(this->winId());
	//QPixmap p = QScreen::grabWindow(this->winId(), 0, 0, 1000, 1000);
	//QPixmap aa = QScreen::grabWindow(this->winId(), 0, 0, 1000, 1000);
	//p.save( "/home/Melda/Desktop/test.jpg" );
	//QSize viewport_size = size();
	//QImage img(viewport_size);
	//QPainter painter(&img);
	//this->render(&painter);
	//img.save("/home/Melda/Desktop/test.png");

	/*grabFrameBuffer();
	cout<<"grabbed the buffer"<<endl;
	QSize viewport_size = size();
	float width = viewport_size.width();
	float height = viewport_size.height();
	...
	if (!bmpBuffer)
	return;
	glReadPixels((GLint)0, (GLint)0, (GLint)width-1, (GLint)height-1, GL_RGB, GL_UNSIGNED_BYTE, bmpBuffer);

	FILE *filePtr = fopen("/home/Melda/Desktop/test.png", "wb");
	if (!filePtr)
	return;*/
}
