...
Simulation::~Simulation(){
	...
	int n = Nodes.size();
	if (!DisplaySave){
		//When reading from save, forces are not recorded, and the matrix is not initiated
		//Therefore, I am deleting it only when the platform is not used for displaying a saved simulation
		for (int i=0;i<n;++i){
...
void Simulation::initiatePrismFromSave(){
	...
	PrismPnt01->updateShapeFromSave(saveFileToDisplay);
...
void  Simulation::updateElementStatesFromSave(){
	...
	for (int i=0; i<currElementNumber; ++i){
		...
		if (shapeType == currShapeType){
			...
			Elements[i]->updateShapeFromSave(saveFileToDisplay);
		}
		else{
