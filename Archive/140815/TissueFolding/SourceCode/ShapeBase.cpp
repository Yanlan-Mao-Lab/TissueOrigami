...
void 	ShapeBase::updateGrowthStrainMatrix(double* growthscale){
	//cout<<"inside update Growth with matrix"<<endl;
	...
	//cout<<"finalised update Growth with matrix"<<endl;
}
...
void	ShapeBase::calculateForces(int RKid, double **SystemForces, vector <Node*>& Nodes){
	...
	boost::numeric::ublas::vector<double> GrowthForces = zero_vector<double>(nMult);
	//displayMatrix(BE,"BE");
	//displayMatrix(GrowthStrain,"GrowthStrain");
	...
	boost::numeric::ublas::axpy_prod(BE,GrowthStrain,GrowthForces);
	//cout<<"getting the net forces"<<endl;
	Forces = Forces - GrowthForces;
