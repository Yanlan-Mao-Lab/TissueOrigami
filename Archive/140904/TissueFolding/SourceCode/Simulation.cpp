...
void Simulation::updateTensionCompressionFromSave(){
	int n = Elements.size();
	for (int i=0;i<n;++i){
		saveFileToDisplayTenComp.read((char*) &Elements[i]->Strain(0), sizeof Elements[i]->Strain(0));
		saveFileToDisplayTenComp.read((char*) &Elements[i]->Strain(1), sizeof Elements[i]->Strain(1));
		saveFileToDisplayTenComp.read((char*) &Elements[i]->Strain(2), sizeof Elements[i]->Strain(2));
		saveFileToDisplayTenComp.read((char*) &Elements[i]->GrowthStrain(0), sizeof Elements[i]->GrowthStrain(0));
		saveFileToDisplayTenComp.read((char*) &Elements[i]->GrowthStrain(1), sizeof Elements[i]->GrowthStrain(1));
		saveFileToDisplayTenComp.read((char*) &Elements[i]->GrowthStrain(2), sizeof Elements[i]->GrowthStrain(2));
	}
}
...
void Simulation::runOneStep(){
	...
	//if (timestep == 100){
		//This will grow the system with a growth matrix
		//double scale[3] = {1.0,0.0,0.0};
		//for ( int i = 0; i < Elements.size(); ++i ){
		//	Elements[i]->updateGrowthStrainMatrix(scale);
		//	Elements[i]->displayMatrix(Elements[i]->GrowthStrain,"Groethstrain");
		//}
		//This will grow the system by modifying the reference shape directly
		//This requires re-calculation of k, and B for every growth increment 10 times slower for a large system with growth
		//for ( int i = 0; i < Elements.size(); ++i ){
		//	Elements[i]->growShape(2.0,2.0,2.0);
		//}
	//}
	const int n = Nodes.size();
	const int dim = 3;
...
void Simulation::writeTensionCompression(){
	...
	for (int i=0;i<n;++i){
		...
		saveFileTensionCompression.write((char*) &Elements[i]->GrowthStrain(0), sizeof Elements[i]->GrowthStrain(0));
		saveFileTensionCompression.write((char*) &Elements[i]->GrowthStrain(1), sizeof Elements[i]->GrowthStrain(1));
		saveFileTensionCompression.write((char*) &Elements[i]->GrowthStrain(2), sizeof Elements[i]->GrowthStrain(2));
	}
}
	...
	if(simTime > initTime && simTime < endTime ){
		...
		for ( int i = 0; i < n; ++i ){
			...
			if (dmag2 > innerRadius2 && dmag2 < outerRadius2){
				...
				//growing the shape
				Elements[i]->growShape(growthscale);
				//This value is stored as fraction per hour, conversion is done by a time scale variable:
				float timescale = 60*60/dt;
