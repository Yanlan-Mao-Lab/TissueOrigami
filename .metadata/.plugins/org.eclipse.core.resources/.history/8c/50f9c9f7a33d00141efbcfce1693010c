...
void 	ShapeBase::getPlasticStrain(int type, float &StrainMag){
	...
	if (type == 0){
		for (int i=0; i<3; ++i){
			StrainMag += CurrPlasticStrainsInGrowthAxesMat(i,i);
		}
		StrainMag /= 3;
	}
	else if (type == 1){
		StrainMag = CurrPlasticStrainsInGrowthAxesMat(0,0);
	}
	else if (type == 2){
		StrainMag = CurrPlasticStrainsInGrowthAxesMat(1,1);
	}
	else if (type == 3){
		StrainMag = CurrPlasticStrainsInGrowthAxesMat(2,2);
	}
	else{
		return;
	}
...
void 	ShapeBase::changeShape(double shapechangescale, int axis){
	...
	//get the current shape change strain in growth axes
	...
	calculateRotationMatrixReferenceToGrowth();
	//need a flag for not calculating the same things over and over again
	calculateCurrShapeChangeStrainsInGrowthAxes();
	//update shape change strain to add this step, for this shape change function

	float effect =  (1.0 + CurrShapeChangeStrainsInGrowthAxesMat(axis,axis)) * shapechangescale;
...
void ShapeBase::calculatePlasticStrain(){
	...
	//cout<<"entering calculateCurrPlasticStrainsInGrowthAxes"<<endl;
	calculateCurrGrowthStrainsInGrowthAxes();
	//cout<<"entering updateGrowthStrainInGrowthAxes"<<endl;
	updateGrowthStrainInGrowthAxes(CurrGrowthStrainAddition);
	//Here I need to update shape change
	updateShapeChangeStrainInGrowthAxes();
	//now sum up all the plastic strains
	updatePlasticStrainInGrowthAxes();
	...
	updatePlasticStainInWorldAxes();
	//cout<<"entering updateLocalPlasticStrain"<<endl;
	updateLocalPlasticStrains();
	//cout<<"finished"<<endl;
}

void 	ShapeBase::calculateCurrGrowthStrainsInGrowthAxes(){
	using namespace boost::numeric::ublas;
	matrix<double> tmpMat1(3,3);
	axpy_prod(RefToGrowthRotMat,LocalGrowthStrainsMat,tmpMat1);
	axpy_prod(tmpMat1,RefToGrowthRotMatT,CurrGrowthStrainsInGrowthAxesMat);
}

void 	ShapeBase::calculateCurrShapeChangeStrainsInGrowthAxes(){
	using namespace boost::numeric::ublas;
	matrix<double> tmpMat1(3,3);
	axpy_prod(RefToGrowthRotMat,LocalShapeChangeStrainsMat,tmpMat1);
	axpy_prod(tmpMat1,RefToGrowthRotMatT,CurrShapeChangeStrainsInGrowthAxesMat);
}

void 	ShapeBase::updateGrowthStrainInGrowthAxes(double* strainsToAdd){
	...
	//displayMatrix(CurrPlasticStrainsInGrowthAxesMat,"CurrPlasticStrainsInGrowthAxesMat");
	CurrGrowthStrainsInGrowthAxesMat(0,0) = ( (1.0 + CurrGrowthStrainsInGrowthAxesMat(0,0)) * (1.0 + strainsToAdd[0]) ) - 1.0;
	CurrGrowthStrainsInGrowthAxesMat(1,1) = ( (1.0 + CurrGrowthStrainsInGrowthAxesMat(1,1)) * (1.0 + strainsToAdd[1]) ) - 1.0;
	CurrGrowthStrainsInGrowthAxesMat(2,2) = ( (1.0 + CurrGrowthStrainsInGrowthAxesMat(2,2)) * (1.0 + strainsToAdd[2]) ) - 1.0;
	//displayMatrix(CurrGrowthStrainsInGrowthAxesMat,"CurrGrowthStrainsInGrowthAxesMat");
}

void 	ShapeBase::updateShapeChangeStrainInGrowthAxes(){
	//Now I have the current strains defining the size of the element in gorwth axis coordinates
	//I can grow the system, with respect to its current size now (10% growth will be 10% of current size, cumulative interest)
	//displayMatrix(CurrPlasticStrainsInGrowthAxesMat,"CurrPlasticStrainsInGrowthAxesMat");
	CurrShapeChangeStrainsInGrowthAxesMat(0,0) += CurrShapeChangeToAdd[0];
	CurrShapeChangeStrainsInGrowthAxesMat(1,1) += CurrShapeChangeToAdd[1];
	CurrShapeChangeStrainsInGrowthAxesMat(2,2) += CurrShapeChangeToAdd[2];
	//displayMatrix(CurrShapeChangeStrainsInGrowthAxesMat,"CurrShapeChangeStrainsInGrowthAxesMat");
}

void 	ShapeBase::updatePlasticStrainInGrowthAxes(){
	//This should be new growth strain in growth axes, not plastic strain to add
	CurrPlasticStrainsInGrowthAxesMat = CurrGrowthStrainsInGrowthAxesMat + CurrShapeChangeStrainsInGrowthAxesMat;
}
...
void 	ShapeBase::updateLocalPlasticStrains(){
	...
	axpy_prod(RefToGrowthRotMatT,CurrPlasticStrainsInGrowthAxesMat,tmpMat1);
	axpy_prod(tmpMat1,RefToGrowthRotMat,LocalPlasticStrainsMat);
	axpy_prod(RefToGrowthRotMatT,CurrGrowthStrainsInGrowthAxesMat,tmpMat1);
	axpy_prod(tmpMat1,RefToGrowthRotMat,LocalGrowthStrainsMat);
	axpy_prod(RefToGrowthRotMatT,CurrShapeChangeStrainsInGrowthAxesMat,tmpMat1);
...
void 	ShapeBase::updatePlasticStainInWorldAxes(){
	boost::numeric::ublas::matrix<double> tmpMat1(3,3);
	boost::numeric::ublas::matrix<double> tmpMat2(3,3);
	axpy_prod(GrowthToWorldRotMat,CurrPlasticStrainsInGrowthAxesMat,tmpMat1);
	boost::numeric::ublas::matrix<double> GrowthToWorldRotMatT(3,3);
	GrowthToWorldRotMatT = trans(GrowthToWorldRotMat);
...
void ShapeBase::calculateRotationMatrixReferenceToGrowth(){
	...
	double* v = new double[3];
	v[0]=GrowthCoordinateSystem[0];
	v[1]=GrowthCoordinateSystem[1];
	v[2]=GrowthCoordinateSystem[2];
	double c, s;
	calculateRotationAngleSinCos(u,v,c,s);
	double *rotAx;
	rotAx = new double[3];
