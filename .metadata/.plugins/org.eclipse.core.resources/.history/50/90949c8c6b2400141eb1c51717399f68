...
void Simulation::growSystemWithMatrix(){
	//cout<<"growing system with matrix"<<endl;
	float centre[2] = {60.0, -1.0};
	float radius = {3.0};
	float MaxValue[3] = {5.55556e-05,5.55556e-05,5.55556e-05};
	...
	for ( int i = 0; i < n; ++i ){
		...
		if (dmag2 < radius2){
			...
			Elements[i]->updateGrowthStrainMatrix(growthscale);
	...
	//cout<<"finalised growing system with matrix"<<endl;
};



void Simulation::runOneStep(){
	cout<<"time : "<<timestep * dt<<", timestep: "<<timestep<<endl;
	cleanreferenceupdates();
	if(nGrowthFunctions>0){
		growSystem();
	}
	growSystemWithMatrix();
	if (timestep == 100){
