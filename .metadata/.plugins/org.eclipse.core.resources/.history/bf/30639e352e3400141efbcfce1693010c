...
bool ModelInputObject::readShapeChangeType1(ifstream& file){
	...
	file >> currHeader;
	if(currHeader == "AxisOfEffect(x,y,z,xy,xz,yz):"){
		string axisOfEffect;
		file >>  axisOfEffect;
		int axis;
		if ( axisOfEffect == "x"){
		...
		else if ( axisOfEffect == "y"){
			axis = 1;
		}
		else if ( axisOfEffect == "z"){
			axis = 2;
		}
		else if ( axisOfEffect == "xy"){
			axis = 3;
		}
		else if ( axisOfEffect == "xz"){
			axis = 4;
		}
		else if ( axisOfEffect == "yz"){
			axis = 5;
		}
		else {
			cerr<<"Error in reading shape change axis, the options are {x,y,z,xy,xz,yz}, current input: "<<axisOfEffect<<endl;
			return false;
		}
	...
	if(currHeader == "MaxValue(fractionPerHour):"){
		...
		cerr<<"Error in reading shape cheange options, curr string: "<<currHeader<<", should have been: MaxValue(fractionPerHour-xyz):" <<endl;
		return false;
	}
	return true;
}
