...
protected:
	...
	double* PlasticStrainsToAddInGrowthAxes;
	bool IsGrowing;
	...
	virtual void calculateZVecForGrowhtAlignment(double* u){ParentErrorMessage();};
	virtual void calculateXVecForGrowhtAlignment(double* u){ParentErrorMessage();};
	void calculateGrowthCoordinateSystem();
	void updateGrowthToWorldRotMat(double* rotMat);
	void calculateRotationMatrixReferenceToGrowth();
	void calculateCurrGrowthStrainsInGrowthAxes();
	void calculateCurrShapeChangeStrainsInGrowthAxes();
	void updateLocalGrowthStrain(double* strainsToAdd);
	void updateLocalPlasticStrains();
	void updatePlasticStrainInGrowthAxes();
	void updateGrowthStrainInGrowthAxes(double* strainsToAdd);
	void updatePlasticStainInWorldAxes();
	void updateShapeChangeStrainInGrowthAxes();
	...
	boost::numeric::ublas::matrix<double> GrowthToWorldRotMat;

	boost::numeric::ublas::matrix<double> GrowthToRefRotMat;
	boost::numeric::ublas::matrix<double> GrowthToRefRotMatT;

	boost::numeric::ublas::matrix<double> RefToGrowthRotMat;
	boost::numeric::ublas::matrix<double> RefToGrowthRotMatT;
	double E, v;
...
public:
	...
	//boost::numeric::ublas::vector<double> LocalGrowthStrain;
	...
	boost::numeric::ublas::matrix<double> CurrGrowthStrainsInGrowthAxesMat;
	boost::numeric::ublas::matrix<double> CurrShapeChangeStrainsInGrowthAxesMat;
	boost::numeric::ublas::matrix<double> CurrPlasticStrainsInGrowthAxesMat;
	double CurrShapeChangeToAdd[3];
	int alingmentTurn;
	double* CurrentNormal;
	bool updatedReference;
	double* GrowthCoordinateSystem;
	...
	void growShape(double* growthscale);
	void growShapeOLD(double* growthscale);
