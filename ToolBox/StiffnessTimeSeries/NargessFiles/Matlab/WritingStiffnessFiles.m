...
for fileID=1:noFiles
    ...
    filename=sprintf('Stiffness96hrRectangleWing_FineGrid_Reduction_%d',fileID-1);
    filename = fopen(filename,'w');
    fprintf(filename,'%d %d \n',[noColumns,noRows]);  % number of columns and rows in the grid matrix
    % writing stiffness file for folding case
    for i=1:noRows
...
for fileID=1:noFiles
    ...
    filename=sprintf('Stiffness96hrRectangleWing_VeryFineGrid_Reduction_%d',fileID-1);
    filename = fopen(filename,'w');
    fprintf(filename,'%d %d \n',[noColumns,noRows]);  % number of columns and rows in the grid matrix
    % writing stiffness file for folding case
    for i=1:noRows
