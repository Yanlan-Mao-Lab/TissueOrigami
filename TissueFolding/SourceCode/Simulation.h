...
private:
	...
	ModelInputObject* ModInp; 								///< The ModelInputObject pointer, used to read the user input file.
	...
	std::ifstream inputMeshFile;							///< The input mesh
	std::ifstream saveFileToDisplayTenComp;					///< Input file to display of saved strains.
	...
	std::ifstream saveFileToDisplaySimSum;					///< Input file to display of saved simulation inputs summary.
	...
	int		anteriorTipIndex;								///< The Node#Id for the anterior tip (+y) of the tissue on xy plane.
	int		posteriorTipIndex;								///< The Node#Id for the posterior tip (+y) of the tissue on xy plane.
	double	StretchDistanceStep;							///< The step of the stretcher movement in micro-meters per time step.
	bool	recordForcesOnFixedNodes;						///< Boolean stating if the forces on fix nodes are recorded.
	...
	std::vector <int> AblatedNodes;							///< The vector of Node#Id values for ablated nodes.
	...
	void removeElementFromEndOfList();								///< This function removes one element form the end of the vector of elements, Simulation#Elements
	void updateNodeNumberFromSave();								///< This function updates the node number of the system according to data read from save file.
	void updateNodePositionsFromSave();								///< This function updates the node positions of the system according to data read from save file.
	void updateElementStatesFromSave();								///< This function updates the element stated of the system according to data read from save file.
	void updateForcesFromSave();									///< This function updates the forces on nodes from save file.
	...
	bool isColumnarLayer3D();										///< This function will check if the input mesh tissue is 3D.
	bool checkIfThereIsPeripodialMembrane();						///< This function will check if the input mesh contains a peripodial membrane.
	void setLinkerCircumference();									///< This function will set the circumference mid line band of linker elements.
	bool calculateTissueHeight();									///< This function will calculate the tissue height of the input mesh.
	...
	void calculateDiscretisationLayers(double &hColumnar, int& LumenHeightDiscretisationLayers, double &hLumen, double &peripodialHeight, int& peripodialHeightDiscretisationLayers, double& hPeripodial); ///< This function calculates how many discretisation layers will be used for lumen and peripodial
	int  countPeripodialHeightDiscretisaionLayers();				///< This function returns the discretisation layers of the peripodial membrane
	void fillColumnarBasedNodeList(vector< vector<int> > &ColumnarBasedNodeArray, vector <int> &ColumnarCircumferencialNodeList); ///< This function will fill the array of node Ids that strt from the  base moving to apical of the columnar layer, at the circumference.
	...
	void calculateNewNodePosForPeripodialNodeAddition(int nodeId0, int nodeId1, std::array<double,3> &pos, double sideThickness); ///< This function calculates the position of a new node to construct a new element at tissue border, for peripodial membrane addition.
	void calculateNewNodePosForPeripodialNodeAddition(int nodeId0, int nodeId1, int nodeId2, std::array<double,3> &pos, double sideThickness); ///< This function calculates the position of a new node to construct a new element at tissue border, for peripodial membrane addition.
	void addNodesForPeripodialOnOuterCircumference (vector< vector<int> > &ColumnarBasedNodeArray, vector< vector<int> > &OuterNodeArray, int LumenHeightDiscretisationLayers, double hLumen, int peripodialHeightDiscretisationLayers, double hPeripodial); 					///< This function is used while adding peripodial membrane to the tissue. It adds the nodes on the outer circumference of peripodial layer.
	void addNodesForPeripodialOnColumnarCircumference (vector< vector<int> > &ColumnarBasedNodeArray, int LumenHeightDiscretisationLayers, double hLumen, int peripodialHeightDiscretisationLayers, double hPeripodial);														///< This function is used while adding peripodial membrane to the tissue. It adds the nodes on the circumference of peripodial, corresponding to the circumference of columnar layer.
	void addLateralPeripodialElements(int LumenHeightDiscretisationLayers, int peripodialHeightDiscretisationLayers, vector< vector<int> > &ColumnarBasedNodeArray, vector< vector<int> > &OuterNodeArray);																		///< This function is used while adding peripodial membrane to the tissue. It adds the elements at the circumference of the peripodial layer, formed by the nodes generated in Simulation#addNodesForPeripodialOnOuterCircumference and Simulation#addNodesForPeripodialOnColumnarCircumference.
	void addNodesForPeripodialOnCap(vector< vector<int> > &ColumnarBasedNodeArray, vector< vector<int> > &PeripodialCapNodeArray, int TissueHeightDiscretisationLayers, int LumenHeightDiscretisationLayers, int peripodialHeightDiscretisationLayers, double hPeripodial);		///< This function is used while adding peripodial membrane to the tissue. It adds the elements on the peripodial cap, reflecting the columnar surface.
	void constructTriangleCornerListOfApicalSurface( vector< vector<int> > &TriangleList);																																														///< This function generates the triangle connectivity of the columnar apical surface, to generate a mimicking peripodial surface.
	void addCapPeripodialElements( vector< vector<int> > &TriangleList, vector< vector<int> > &PeripodialCapNodeArray, int peripodialHeightDiscretisationLayers);																												///< This function is used while adding peripodial membrane to the tissue. It adds the nodes on the peripodial cap, reflecting the columnar surface.
	void correctCircumferentialNodeAssignment(vector< vector<int> > OuterNodeArray);												///< This function corrects the circumferential node assignment after adding new lateral elements to the tissue.
	...
	bool bindPeripodialToColumnar();								///< This function binds the positions of periopodial and columnar apical surfaces.
	...
	void recordForcesOnClampBorders();								///< This function records the forces on clamped borders, to do : delete?
	...
	void clearLaserAblatedSites();									///< This function clears the elements with laser ablated nodes.
    void pokeElement(int elementId, double dx, double dy, double dz);	///< This function allows the user to poke an element, to check for stability of the emergent topology.
    void addCurvatureToColumnar(double h);							///< This function adds curvature to the apical surface of the columnar layer, and an equivalent negative curvature to peripodial apical layer if available. This function is used to generate a lumen in the simple cap peripodial ECM simulations.
    void thickenECM();												///< This function is used toe alter the thickkness of the ECM manually, not called during normal sumulations, but can be used if desired.
	...
	void clearScaleingDueToApikobasalRedistribution();				///< This function sets volume redistribution scaling to zero.
...
public:
	 ...
	 */
	bool CircumferentialNodeFix[5][3]; 							///< The double array of booleans stating if the circumference is fixed with viscosity: row 0: apical circumferece x,y,z ; row 1: basal circumference x,y,z; row 2: linker apical circumference x,y,z, row 3: linker basal circumference x,y,z, row 4: all circumference x,y,z
	...
	//double currViscMidline;									///< The calculated value of viscosity in the mid-line of tissue, obtained from apical and absal values.
	double PeripodialThicnessScale;								///< The scale of peripodial thickness wrt the columnar thickness.
	...
	Lumen* tissueLumen;											///< This is the pointer to tissue lumen object.
	double lumenHeight;											///< The height of the lumen, micro-meters.
	double lumenHeightScale;									///< The scale of the height of the lumen, wrt the thickness of the columnar layer.
	...
	bool 	adherePeripodialToColumnar;		///< The boolean stating if the peripodial and columnar apical surfaces are adhered to each other at the beginning of simulation.
	...
	vector <double> drawingPointsX, drawingPointsY, drawingPointsZ; ///< The coordinates of points for debugging puposes.

	bool thereIsExplicitECM;    			///< Boolean stating if there is explicit ECM definition in the system.
	...
	bool thereIsExplicitActin;				///< Boolean stating if there is explicit actin definition in the system.
	bool thereIsExplicitLumen;				///< Boolean stating if there is explicit lumen definition in the system.
	double lumenBulkModulus;				///< The Young's modulus of lumen
	double lumenGrowthFold;					///< The growth fold of the lumen per 24 hours
	...
	bool readExectuableInputsGUI(string inputfile, string read_in_directory, string output_directory=".");
	...
	void wrapUpAtTheEndOfSimulation();									///< Correct tissue alignment once more before simulation is finalised to have the correct final mesh.

	void writeRelaxedMeshFromCurrentState();
	void writeMeshRemovingAblatedRegions();
	void calculateDVDistance();											///< Calculate the long (DV) axis tip to tip distance (not contour length), the DV axis is defined by Simulation#ventralTipIndex and Simulation#dorsalTipIndex, as assigned in Simulation#assignTips.
	...
	void addNodesForSideECMOnOuterCircumference (vector< vector<int> > &ColumnarBasedNodeArray, vector< vector<int> > &OuterNodeArray);		///< This function adds the nodes for the side ECM layer.
