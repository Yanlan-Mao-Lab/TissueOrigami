...
public:
    ...
    void                    calculateShapeChangeIncrementFromGridCorners(int ShapeChangeGridInterpolationType, double dt, GrowthFunctionBase* currSCF, gsl_matrix* increment, int sourceTissue,  int IndexX, int IndexY, double FracX, double FracY);   ///< This function will calculate the current shape change deformation gradient increment due to elemental active shape change, from input shape change grids.
    ...
    bool                                        isGridBasedShapeChangeAppliedToElement(bool applyColumnar, bool applyPeripodial, bool applyBasalECM, bool applyToLateralECM, bool applyApically, bool applyBasally, bool applyMidLayer);   ///< This function decides if grid based shape change perturbation is applied to this element
